					      ...
					      uint32_t flags)
{
	...
	return sys_mm_drv_update_region_flags((__sparse_force void *)vma, size, flags | K_MEM_DIRECT_MAP);
}
						...
						const struct sof_man_module *mod)
{
	...
	if (sys_mm_drv_map_region((__sparse_force void *)va_base, POINTER_TO_UINT(NULL),
				  bss_size, SYS_MM_MEM_PERM_RW | K_MEM_DIRECT_MAP) < 0)
		return -ENOMEM;
...
#if CONFIG_USERSPACE
	bool is_userspace = !!drv->drv_heap;
#endif
	struct userspace_module *userspace = NULL;
	const struct sof_man_module *mod;
...
#if CONFIG_USERSPACE
	if (is_userspace) {
		ret = userspace_module_create(&userspace, drv, mod, agent, module_entry_point,
					      module_id, instance_id, 0, log_handle, &mod_cfg,
					      agent_iface);
		if (ret) {
...
#if CONFIG_USERSPACE
	if (userspace)
		userspace_module_destroy(drv, userspace);
#endif /* CONFIG_USERSPACE */
