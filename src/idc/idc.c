...
/**
* \brief Handle IDC slave core crashed message.
* \param[in] header IDC message header
*/
static void idc_slave_core_crashed(const uint32_t header)
{
	...
	case IDC_SCC_REASON_WATCHDOG:
		watchdog_slave_core_timeout(core);
		break;
#endif
	default:
		;
 ...
 */
	...
	case iTS(IDC_MSG_PREPARE_D0ix):
		idc_prepare_d0ix();
		break;
	case iTS(IDC_MSG_SLAVE_CORE_CRASHED):
		idc_slave_core_crashed(msg->header);
		break;
	default:
