		       ...
		       struct sof_ipc_stream_params *params, notifier_callback_t cb)
{
	...
	comp_err(dev, "%c round_up_size %u, period_count %u, period_bytes %u, buffer_size %u, buffer_size_preferred %u",
		 params->direction == SOF_IPC_STREAM_PLAYBACK ? 'p' :'c',
		 round_up_size, period_count, period_bytes, buffer_size, buffer_size_preferred);
