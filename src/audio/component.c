...
static inline bool comp_check_eos(struct comp_dev *dev)
{
	...
	comp_dev_for_each_producer(dev, buffer) {
		...
		if (state == STREAM_STATE_END_OF_STREAM_FLUSH) {
			...
			sink_state = STREAM_STATE_END_OF_STREAM_FLUSH;
			break;
		} else if (state == STREAM_STATE_END_OF_STREAM) {
			...
			if (source_get_data_available(source) < min_avail) {
				...
				if (dev->ipc_config.proc_domain == COMP_PROCESSING_DOMAIN_DP) {
					...
					sink_state = STREAM_STATE_END_OF_STREAM;
					break;
				}
			}
		}
	...
	if (sink_state != STREAM_STATE_INITIAL) {
		comp_dev_for_each_consumer(dev, buffer)
			audio_buffer_set_state(&buffer->audio_buffer, sink_state);

		/* For STREAM_STATE_END_OF_STREAM_FLUSH process data normally. */
		return sink_state != STREAM_STATE_END_OF_STREAM_FLUSH;
	}
