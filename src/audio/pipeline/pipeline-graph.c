		     ...
		     int dir)
{
	...
	if (dir == PPL_CONN_DIR_COMP_TO_BUFFER)
		comp_info(comp, "connect buffer %p %d as sink", (void *)buffer, buf_get_id(buffer));
	else
		comp_info(comp, "connect buffer %p %d as source", (void *)buffer, buf_get_id(buffer));
...
void pipeline_disconnect(struct comp_dev *comp, struct comp_buffer *buffer, int dir)
{
	...
	if (dir == PPL_CONN_DIR_COMP_TO_BUFFER)
		comp_info(comp, "disconnect buffer %d as sink", buf_get_id(buffer));
	else
		comp_info(comp, "disconnect buffer %d as source", buf_get_id(buffer));
