...
static const struct module_interface userspace_module_adapter_interface;
			      ...
			      struct user_worker_data *worker_data)
{
	...
	buffer = rzalloc(SOF_MEM_FLAG_USER_SHARED, MAX_PARAM_SIZE * 2);
	if (!buffer)
		return -ENOMEM;
...
static int modules_user_init_worker(struct userspace_module *user,
				    k_work_user_handler_t user_handler)
{
	...
	if (!wd) {
		wd = rzalloc(SOF_MEM_FLAG_USER_SHARED, sizeof(*wd));
		if (!wd)
			return -ENOMEM;
	...
	if (!wd->module_ref_cnt) {
		k_work_user_init(&wd->work_item, user_handler);
...
static void modules_user_free_worker(struct userspace_module *user)
{
	uint32_t sd_id = user->security_domain_id;
	struct user_worker_data *wd = worker_data[sd_id];
...
static int module_user_call(struct userspace_module *user, struct processing_module *mod,
	...
	while(1) {
		...
		case MODULE_CMD_SET_CONF:
			params->status = ops->set_configuration(params->mod,
								...
								params->ext.set_conf.data_offset_size,
		...
		case MODULE_CMD_GET_CONF:
			params->status = ops->get_configuration(params->mod,
								...
								params->ext.get_conf.data_offset_size,
...
static int userspace_module_add_sections(struct userspace_module *user, uint32_t instance_id,
...
int userspace_module_create(struct userspace_module **user_mod, const struct comp_driver *drv,
			    const struct sof_man_module *manifest, system_agent_start_fn agent,
			    ...
			    void **adapter)
{
	...
	user = sys_heap_alloc(drv->drv_heap, sizeof(struct userspace_module));
	if (!user)
		return -ENOMEM;
	...
	ret = userspace_module_memory_init(user, drv);
	if (ret)
		goto error_dom;

	ret = userspace_module_add_sections(user, instance_id, manifest);
	if (ret)
		goto error_dom;
	
	ret = modules_user_init_worker(user, module_user_handler);
	if (ret)
		goto error_dom;
	
	struct user_worker_data *wr_data = user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	params->ext.agent.start_fn = agent;
	...
	ret = module_user_call(user, NULL, MODULE_CMD_AGENT_START, params);
	if (ret)
		goto error_worker;
	...
	*user_mod = user;
	...
	((struct comp_driver *)drv)->adapter_ops = &userspace_module_adapter_interface;

	return params->status;

error_worker:
	modules_user_free_worker(user);
error_dom:
	rfree(domain);
error:
	sys_heap_free(drv->drv_heap, user);
	return ret;
}

void userspace_module_destroy(const struct comp_driver *drv, struct userspace_module *mod)
{
	tr_dbg(&modules_user_tr, "userspace free");
	modules_user_free_worker(mod);
	sys_heap_free(drv->drv_heap, mod);
}
 ...
 */
static int userspace_module_init(struct processing_module *mod)
{
	...
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_INIT, params);
	if (ret < 0)
		return ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_prepare(struct processing_module *mod,
				    ...
				    struct sof_sink **sinks, int num_of_sinks)
{
	...
	if (!mod->dev_user->interface->prepare)
		return 0;
	...
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_PREPARE, params);
	if (ret < 0)
		return ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_process(struct processing_module *mod, struct sof_source **sources,
				    int num_of_sources, struct sof_sink **sinks, int num_of_sinks)
{
	return mod->dev_user->interface->process(mod, sources, num_of_sources, sinks, num_of_sinks);
}
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_reset(struct processing_module *mod)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
	...
	if (!mod->dev_user->interface->reset)
		return 0;

	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_RESET, params);
	if (ret < 0)
		return ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_free(struct processing_module *mod)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
	...
	if (mod->dev_user->interface->free) {
		ret = module_user_call(mod->dev_user, mod, MODULE_CMD_FREE, params);
		if (ret < 0)
			return ret;
	}

	/* Destroy workqueue if this was last active userspace module */
	userspace_module_destroy(mod->dev->drv, mod->dev_user);
	mod->dev_user = NULL;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_set_configuration(struct processing_module *mod, uint32_t config_id,
					      enum module_cfg_fragment_position pos,
					      ...
					      size_t response_size)
{
	...
	if (!mod->dev_user->interface->set_configuration)
		return 0;

	params->ext.set_conf.config_id = config_id;
	params->ext.set_conf.pos = pos;
	params->ext.set_conf.data_offset_size = data_offset_size;
	...
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_SET_CONF, params);
	if (!ret)
		ret = params->status;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_get_configuration(struct processing_module *mod, uint32_t config_id,
					      ...
					      size_t fragment_size)
{
	...
	if (!mod->dev_user->interface->get_configuration)
		return -EIO;

	params->ext.get_conf.config_id = config_id;
	params->ext.get_conf.data_offset_size = data_offset_size;
	...
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_GET_CONF, params);
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_set_processing_mode(struct processing_module *mod,
						enum module_processing_mode mode)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	...
	if (!mod->dev_user->interface->set_processing_mode)
		return 0;

	params->ext.proc_mode.mode = mode;
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_SET_PROCMOD, params);
	if (ret < 0)
		return ret;
 ...
 */
static enum module_processing_mode userspace_module_get_processing_mode(struct processing_module *mod)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
	...
	if (!mod->dev_user->interface->get_processing_mode)
		return -EIO;

	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_GET_PROCMOD, params);
	if (ret < 0)
		return ret;
 ...
 */
static bool userspace_module_is_ready_to_process(struct processing_module *mod,
						 struct sof_source **sources,
						 int num_of_sources,
						 struct sof_sink **sinks,
						 int num_of_sinks)
{
	...
	if (!mod->dev_user->interface->is_ready_to_process)
	...
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_PROC_READY, params);
	if (ret < 0)
		return generic_module_is_ready_to_process(mod, sources, num_of_sources, sinks,
							  num_of_sinks);
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_bind(struct processing_module *mod, struct bind_info *bind_data)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
	...
	if (!mod->dev_user->interface->bind)
		return 0;

	params->ext.bind_data = bind_data;
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_BIND, params);
	if (ret < 0)
		return ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_unbind(struct processing_module *mod, struct bind_info *unbind_data)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
	...
	if (!mod->dev_user->interface->unbind)
		return 0;

	params->ext.bind_data = unbind_data;
	ret = module_user_call(mod->dev_user, mod, MODULE_CMD_UNBIND, params);
	if (ret < 0)
		return ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_trigger(struct processing_module *mod, int cmd)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret = 0;
	...
	if (mod->dev_user->interface->trigger) {
		params->ext.trigger_data = cmd;
		ret = module_user_call(mod->dev_user, mod, MODULE_CMD_TRIGGER, params);
		if (ret < 0)
			return ret;
		ret = params->status;
	}
...
APP_USER_DATA static const struct module_interface userspace_module_adapter_interface = {
	.init = userspace_module_init,
	.is_ready_to_process = userspace_module_is_ready_to_process,
	.prepare = userspace_module_prepare,
	.process = userspace_module_process,
	.set_configuration = userspace_module_set_configuration,
	.get_configuration = userspace_module_get_configuration,
	.set_processing_mode = userspace_module_set_processing_mode,
	.get_processing_mode = userspace_module_get_processing_mode,
	.reset = userspace_module_reset,
	.free = userspace_module_free,
	.bind = userspace_module_bind,
