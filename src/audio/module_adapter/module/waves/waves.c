...
static int waves_effect_allocate(struct processing_module *mod)
{
	...
	if (status) {
		comp_err(dev, "MaxxEffect_GetEffectSize returned %d",
			 status);
		return -EINVAL;
	}
	...
	if (!waves_codec->effect) {
		comp_err(dev, "failed to allocate %d bytes for effect",
			 waves_codec->effect_size);
		return -ENOMEM;
	}

	comp_dbg(dev, "allocated %d bytes for effect",
		  waves_codec->effect_size);
...
static int waves_effect_check(struct comp_dev *dev, struct sof_source *source,
			      struct sof_sink *sink)
{
	...
	if (source_get_rate(source) != sink_get_rate(sink)) {
		comp_err(dev, "source %d sink %d rate mismatch",
			 source_get_rate(source), sink_get_rate(sink));
		return -EINVAL;
	}

	/* upmix/downmix not supported */
	if (source_get_channels(source) != sink_get_channels(sink)) {
		comp_err(dev, "source %d sink %d channels mismatch",
			 source_get_channels(source), sink_get_channels(sink));
		return -EINVAL;
	}

	/* different frame format not supported */
	if (source_get_frm_fmt(source) != sink_get_frm_fmt(sink)) {
		comp_err(dev, "source %d sink %d sample format mismatch",
			 source_get_frm_fmt(source), sink_get_frm_fmt(sink));
		return -EINVAL;
	}

	/* different interleaving is not supported */
	if (source_get_buffer_fmt(source) != sink_get_buffer_fmt(sink)) {
		comp_err(dev, "source %d sink %d buffer format mismatch",
			 source_get_buffer_fmt(source), sink_get_buffer_fmt(sink));
		return -EINVAL;
	}

	if (!format_is_supported(source_get_frm_fmt(source))) {
		comp_err(dev, "float samples not supported");
		return -EINVAL;
	}

	if (!layout_is_supported(source_get_buffer_fmt(source))) {
		comp_err(dev, "non interleaved format not supported");
		return -EINVAL;
	}

	if (!rate_is_supported(source_get_rate(source))) {
		comp_err(dev, "rate %d not supported",
			 source_get_rate(source));
		return -EINVAL;
	}

	if (source_get_channels(source) != 2) {
		comp_err(dev, "channels %d not supported",
...
static int waves_effect_init(struct processing_module *mod, struct sof_source *source)
{
	struct comp_dev *dev = mod->dev;
	struct module_data *codec = &mod->priv;
	struct waves_codec_data *waves_codec = codec->private;
	...
	sample_format = format_convert_sof_to_me(source_get_frm_fmt(source));
	if (sample_format < 0) {
		comp_err(dev, "sof sample format %d not supported",
			 source_get_frm_fmt(source));
		return -EINVAL;
	}

	buffer_format = layout_convert_sof_to_me(source_get_buffer_fmt(source));
	if (buffer_format < 0) {
		comp_err(dev, "sof buffer format %d not supported",
			 source_get_buffer_fmt(source));
		return -EINVAL;
	}

	sample_bytes = sample_format_convert_to_bytes(sample_format);
	if (sample_bytes < 0) {
		comp_err(dev, "sample_format %d not supported",
			 sample_format);
		return -EINVAL;
	}
	...
	waves_codec->i_format.sampleRate = source_get_rate(source);
	waves_codec->i_format.numChannels = source_get_channels(source);
	 ...
	 */
	waves_codec->buffer_samples = source_get_rate(source) * dev->pipeline->period /
		1000000;
	waves_codec->buffer_bytes = waves_codec->buffer_samples *
		source_get_channels(source) * waves_codec->sample_size_in_bytes;

	// trace allows printing only up-to 4 words at a time
	// logging all the information in two calls
	comp_info(dev, "rate %d, channels %d", waves_codec->i_format.sampleRate,
		  waves_codec->i_format.numChannels);

	comp_info(dev, "format %d, layout %d, frame %d",
	...
	if (status) {
		comp_err(dev, "MaxxEffect_Initialize returned %d", status);
		return -EINVAL;
	}
...
static int waves_effect_buffers(struct processing_module *mod)
{
	...
	if (!i_buffer) {
		comp_err(dev, "failed to allocate %d bytes for i_buffer",
	...
	if (!o_buffer) {
		comp_err(dev, "failed to allocate %d bytes for o_buffer",
			 waves_codec->buffer_bytes);
		ret = -ENOMEM;
		goto err;
	}
	...
	comp_dbg(dev, "in_buff_size %d, out_buff_size %d",
...
static int waves_effect_revision(struct processing_module *mod)
{
	...
	if (status) {
		comp_err(dev, "MaxxEffect_Revision_Get returned %d",
			 status);
		return -EINVAL;
	}
	...
	void *data, uint32_t size)
{
	...
	if (!waves_codec->config_blob) {
		...
		if (!waves_codec->config_blob) {
			comp_err(dev,
				"failed to allocate %d bytes for config blob",
				size);
			return -ENOMEM;
		}
...
static int waves_effect_apply_config(struct processing_module *mod)
{
	...
	comp_info(dev, "config %p, size %d, avail %d",
		  cfg->data, cfg->size, cfg->avail);

	if (!cfg->data) {
		ret = waves_effect_apply_config_blob_from_cache(mod);
		if (ret) {
			comp_err(dev, "error %x: apply cache fail",
				ret);
			return ret;
		}
	}
	...
	if (cfg->size > MAX_CONFIG_SIZE_BYTES) {
		comp_err(dev, "provided config is too big, size %d",
			 cfg->size);
		return -EINVAL;
	}
	...
	for (index = 0; index < cfg->size && (!ret); param_number++) {
		...
		comp_info(dev, "param num %d id %d size %d",
			  param_number, param->id, param->size);

		if ((param->size <= header_size) || (param->size > MAX_CONFIG_SIZE_BYTES)) {
			comp_err(dev, "invalid module_param size: %d",
				param->size);
			return -EINVAL;
		}

		if ((index + param->size) > cfg->size) {
			comp_err(dev, "module_param size: %d exceeds cfg buffer size: %d",
				param->size, cfg->size);
			return -EINVAL;
		}
...
static int waves_codec_init(struct processing_module *mod)
{
	...
	if (!waves_codec) {
		comp_err(dev, "failed to allocate %d bytes for waves_codec_data",
	...
	if (ret) {
		comp_err(dev, "MaxxEffect_GetMessageMaxSize returned %d", ret);
		return -EINVAL;
	}

	response = mod_alloc_align(mod, waves_codec->response_max_bytes, 16);
	if (!response) {
		comp_err(dev, "failed to allocate %d bytes for response",
			 waves_codec->response_max_bytes);
		return -ENOMEM;
	}
			       ...
			       struct sof_sink **sinks, int num_of_sinks)
{
	...
	ret = waves_effect_check(dev, sources[0], sinks[0]);
	if (ret)
		goto error;

	ret = waves_effect_init(mod, sources[0]);
	if (ret)
		goto error;
		    ...
		    struct output_stream_buffer *output_buffers, int num_output_buffers)
{
	...
	if (codec->mpd.avail != waves_codec->buffer_bytes) {
		comp_warn(dev, "input buffer %d is not full %d",
...
static int waves_codec_reset(struct processing_module *mod)
{
	...
	if (status) {
		comp_err(dev, "MaxxEffect_Reset returned %d", status);
		ret = -EINVAL;
	}
