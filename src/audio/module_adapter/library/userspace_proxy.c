...
LOG_MODULE_REGISTER(userspace_proxy_tr, CONFIG_SOF_LOG_LEVEL);
...
#define HEAP_PART_CACHED
	...
	struct k_mem_partition heap_part_cached = { .attr = K_MEM_PARTITION_P_RW_U_RW };

	k_mem_region_align(&heap_part_cached.start, &heap_part_cached.size,
					...
					const struct sof_man_module *const mod)
{
	...
	mem_partition.attr = K_MEM_PARTITION_P_RW_U_RW;
	return k_mem_domain_add_partition(user->comp_dom, &mem_partition);
}
