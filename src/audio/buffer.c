...
static struct comp_buffer *buffer_alloc_struct(void *stream_addr, uint32_t size, uint32_t flags,
					       bool is_shared)
{
				       ...
				       bool is_shared)
{
	...
	if (preffered_size % minimum_size)
		preffered_size += minimum_size - preffered_size % minimum_size;
...
int buffer_set_size_range(struct comp_buffer *buffer, uint32_t preffered_size,
			  uint32_t minimum_size, uint32_t alignment)
{
	const void* ptr = audio_stream_get_addr(&buffer->stream);
	...
	if (!alignment) {
		for (new_size = preffered_size; new_size >= minimum_size && !new_ptr;
